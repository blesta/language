<?php
/**
 * Transactions
 *
 * @package blesta
 * @subpackage blesta.language.zh_cn
 * @copyright Copyright (c) 2026, Phillips Data, Inc.
 * @license http://www.blesta.com/license/ The Blesta License Agreement
 * @link http://www.blesta.com/ Blesta
 */

$lang['Transactions.!error.amount.applied'] = '交易金额不得低于已应用于发票的金额。请先从一张或多张发票中撤销该交易的应用。';
$lang['Transactions.!error.amount.format'] = '金额必须是数字。';
$lang['Transactions.!error.amounts.applied'] = '无法将“应付金额”值应用到指定的发票上。请重试。';
$lang['Transactions.!error.amounts.overage'] = '一个或多个 "待付金额 "值无法应用到指定发票。确保 "待付金额 "不超过发票上的 "应付金额"，发票未结清，且 "待付金额 "值的总和不超过 "付款金额 "或 "贷记金额"。';
$lang['Transactions.!error.amounts.positive'] = '一个或多个 "待付金额 "值为负数。确保每个 "待付金额 "值为零或以上。';
$lang['Transactions.!error.client_id.exists'] = '客户 ID 无效。';
$lang['Transactions.!error.currency.length'] = '货币长度必须为 3 个字符。';
$lang['Transactions.!error.currency.mismatch'] = '给定货币与至少一张相关发票不匹配。';
$lang['Transactions.!error.currency.missing'] = '请设置货币。';
$lang['Transactions.!error.date_added.format'] = '交易日期不能是未来日期。';
$lang['Transactions.!error.date.format'] = '申请的日期无效。';
$lang['Transactions.!error.gateway_id.exists'] = '网关 ID 无效。';
$lang['Transactions.!error.invoice_id.exists'] = '发票 ID 无效。';
$lang['Transactions.!error.is_lang.format'] = 'is_lang 必须是一个数字。';
$lang['Transactions.!error.is_lang.length'] = 'is_lang 长度不得超过 1 个字符。';
$lang['Transactions.!error.message.length'] = '信息长度不得超过 255 个字符。';
$lang['Transactions.!error.name.empty'] = '请输入姓名。';
$lang['Transactions.!error.name.length'] = '名称长度不得超过 32 个字符。';
$lang['Transactions.!error.parent_transaction_id.length'] = '父事务 ID 长度不得超过 128 个字符。';
$lang['Transactions.!error.reference_id.length'] = '参考编号长度不得超过 128 个字符。';
$lang['Transactions.!error.status.format'] = '交易状态无效。';
$lang['Transactions.!error.transaction_id.currency_matches'] = '发票金额与交易货币不符，因此无法应用。';
$lang['Transactions.!error.transaction_id.exists'] = '交易 ID 无效。';
$lang['Transactions.!error.transaction_id.length'] = '交易 ID 长度不得超过 128 个字符。';
$lang['Transactions.!error.transaction_type_id.exists'] = '交易类型 ID 无效。';
$lang['Transactions.!error.type_id.exists'] = '交易类型 ID 无效。';
$lang['Transactions.!error.type.format'] = '交易类型无效。';
$lang['Transactions.!error.type.valid'] = '请选择有效类型。';
$lang['Transactions.debit_types.credit'] = '信用';
$lang['Transactions.debit_types.debit'] = '借记';
$lang['Transactions.status.approved'] = '批准';
$lang['Transactions.status.declined'] = '拒绝';
$lang['Transactions.status.error'] = '错误';
$lang['Transactions.status.pending'] = '待定';
$lang['Transactions.status.refunded'] = '退款';
$lang['Transactions.status.returned'] = '已返回';
$lang['Transactions.status.void'] = '无效';
$lang['Transactions.types.ach'] = 'ACH';
$lang['Transactions.types.cc'] = '信用卡';
$lang['Transactions.types.other'] = '其他';

