<?php
/**
 * Config
 *
 * @package blesta
 * @subpackage blesta.language.tr_tr
 * @copyright Copyright (c) 2026, Phillips Data, Inc.
 * @license http://www.blesta.com/license/ The Blesta License Agreement
 * @link http://www.blesta.com/ Blesta
 */

$lang['Config.!error.domain_duplicated'] = '"%1$s" alan adı zaten sepetinizde bulunuyor.';
$lang['Config.!error.invalid_domain_term'] = '"%1$s" etki alanı için seçilen terim geçersiz.';
$lang['Config.!error.invalid_module'] = 'Paket şu anda kullanılamıyor.';
$lang['Config.!error.invalid_pricing_id'] = 'Verilen paket bulunamadı.';
$lang['Config.!error.invalid_quantity_option'] = '"%1$s" seçenek değeri %2$s ve %3$s arasında olmalıdır.';
$lang['Config.!error.invalid_value_option'] = '%1$s için bir değer seçilmelidir.';
$lang['Config.index.field_addon_label'] = '%1$s (%2$s @ %3$s)';
$lang['Config.index.field_addon_label_recurring'] = '%1$s (%2$s @ %3$s, yenileme @ %4$s)';
$lang['Config.index.field_default_addon'] = 'Hiçbiri';
$lang['Config.index.field_module_group_id'] = 'Grup';
$lang['Config.index.package_price'] = '%1$s @ %2$s';
$lang['Config.index.package_price_breakdown'] = '%1$s %2$s (%3$s/mo)';
$lang['Config.index.package_price_breakdown_savings'] = '%1$s %2$s ( /mo  tasarruf!)%3$s %4$s%% ';
$lang['Config.index.package_price_recurring'] = '%1$s @ %2$s (%3$s tarihinde yenilenir)';
$lang['Config.index.package_price_recurring_breakdown'] = '%1$s %2$s ( %3$s adresini yeniler) (%4$s/ay) ';
$lang['Config.index.package_price_recurring_breakdown_savings'] = '%1$s %2$s ( \'de yenileniyor) ( /ay  tasarruf!) %3$s%4$s %5$s%% ';

